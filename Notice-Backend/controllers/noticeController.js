...
//   try {
//     const { type, fullName, oldName, newName, content } = req.body;
...
//     res.json(notice); // this already includes `paid: true/false`
//   } catch (err) {
...
//   try {
//     const notices = await Notice.find({ user: req.user.id }).sort({
//       createdAt: -1,
//     });
//     res.json(notices);
//   } catch (err) {
...
//     res.json({ message: "Notice rejected" });
//   } catch (err) {
...
//     res
//       .status(500)
//       .json({ message: "Failed to load notices", error: err.message });
//   }
// };
...
//     const notice = await Notice.findOne({ referenceId: ref }).populate(
//       "user",
//       "name email"
//     );
...
//         { oldName: { $regex: search, $options: "i" } },
//         { newName: { $regex: search, $options: "i" } },
//         { referenceId: { $regex: search, $options: "i" } },
//         { type: { $regex: search, $options: "i" } },
//       ],
//     };

//     const notices = await Notice.find(query).sort({ publishAt: -1 });
